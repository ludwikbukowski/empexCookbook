defmodule EmpexCookbookWeb.ProfileControllerTest do
  @moduledoc false
  ...
  def fixture(:user1) do
    {:ok, user} = EmpexCookbook.Accounts.Auth.register(@user1_attrs)
    user
  end

  def fixture(:user2) do
    {:ok, user} = EmpexCookbook.Accounts.Auth.register(@user2_attrs)
    user
  end

  def secure_conn(conn) do
    user = fixture(:user1)
    {:ok, jwt, _} = user |> EmpexCookbookWeb.Guardian.encode_and_sign(%{}, token_type: :token)
