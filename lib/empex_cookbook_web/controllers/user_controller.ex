defmodule EmpexCookbookWeb.UserController do
  use EmpexCookbookWeb, :controller
  use EmpexCookbookWeb.GuardedController

  alias EmpexCookbook.Accounts.{Auth, Users}
  ...
  def create(conn, %{"user" => user_params}, _) do
    case Auth.register(user_params) do
      ...
      {:error, changeset} ->
        render(conn, EmpexCookbookWeb.ChangesetView, "error.json", changeset: changeset)
    end
  end

  def current_user(conn, _params, user) do
    jwt = EmpexCookbookWeb.Guardian.Plug.current_token(conn)
